...
const schema = Joi.object({
    idPartnerships: Joi.number().required().messages({
        'number.base': 'Invalid Partnership',
        'any.required': 'Partnership is required'
    }),
...
async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    if (req.method === 'POST') {
        ...
        form.parse(req, async (err, fields, files) => {
            ...
            const idPartnerships = fields.idPartnerships ? parseInt(fields.idPartnerships[0]) : null;
            ...
            try {
                const existing = await Partnership.findByPk(idPartnerships!, { transaction });
                if (!existing) {
                ...
                return res.status(200).json({ success: true, message: 'Partnership updated successfully', data: existing });
            } catch (e) {
                await transaction.rollback();
