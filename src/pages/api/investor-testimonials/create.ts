    ...
    if (req.method === 'POST') {
        ...
        form.parse(req, async (err, fields, files) => {
            ...
            try {
                ...
                const testimonial = await InvestorTestimonial.create({
                    name: data.name,
                    image: imageFileName,
                    rating: data.rating,
                ...
                return res.status(200).json({ success: true, message: 'Investor testimonial created successfully', data: testimonial });
            } catch (e) {
                await transaction.rollback();
